...
print("Initializing Dense Retriever and building indexes (if not already built)...")
dense_retriever_main = DenseRetriever(storage_path_prefix=DENSE_STORAGE_PATH)
dense_retriever_main.pre_build_all_paper_indexes(review_chunks_df_main, force_rebuild=False) # Set force_rebuild=True if needed

# Initialize RAG System with the Dense Retriever
rag_sys = RAGSystem(retriever_instance=dense_retriever_main)
