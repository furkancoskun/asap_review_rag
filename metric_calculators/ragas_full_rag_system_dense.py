...
FULL_RAGAS_RESULTS_FILE = "ragas_results/ragas_full_system_dense_results.csv"
...
eval_queries_df = eval_inputs_df[eval_inputs_df['retriever_type'] == 'Dense (all-MiniLM-L6-v2)'].copy()
...
print("\nInitializing Dense Retriever and RAG System...")
dense_retriever_eval = DenseRetriever(storage_path_prefix=DENSE_STORAGE_PATH)
dense_retriever_eval.pre_build_all_paper_indexes(review_chunks_df, force_rebuild=False)

rag_system_eval = RAGSystem(retriever_instance=dense_retriever_eval)

GENERATED_ANSWERS_FILE = f"{DATA_SAVE_DIR}/generated_answers_dense_rag.csv"
all_generated_data = []
...
if full_ragas_summary_df is None:
    try:
        ...
        full_ragas_summary_df['retriever_type'] = "Dense (all-MiniLM-L6-v2)"
...
if full_ragas_summary_df is not None and not full_ragas_summary_df.empty:
    print("\n--- Final Full RAG System RAGAS Summary (Dense Retriever) ---")
